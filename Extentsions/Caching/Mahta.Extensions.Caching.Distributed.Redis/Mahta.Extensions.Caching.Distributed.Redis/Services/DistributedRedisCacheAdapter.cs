...
namespace Mahta.Extensions.Caching.Distributed.Redis.Services;
...
public class DistributedRedisCacheAdapter : ICacheAdapter
{
    ...
    public TOutput Get<TOutput>(string key)
    {
        ...
        {
            _logger.LogTrace("DistributedCache Redis Adapter Successful Get Cache with key : {key} and data : {data}", key, _serializer.Serialize(result));
