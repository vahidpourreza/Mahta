...
namespace Mahta.Extensions.Caching.InMemory.Services;
...
public class InMemoryCacheAdapter : ICacheAdapter
{
    ...
    public TOutput Get<TOutput>(string key)
    {
        ...
        if (result)
            _logger.LogTrace("InMemoryCache Adapter Successful Get Cache with key : {key} and data : {data}", key, _jsonSerializer.Serialize(resultObject));
        else
